...
如果暂时没有可用的数据线，也可以把 APK 文件发送到手机并在手机上手动安装。注意不要直接安装已经失效的微信临时文件路径；如果手机提示 `open failed: ENOENT`，通常表示临时文件已经不存在，需要先把 APK 保存到手机本地文件夹，再从文件管理器打开安装。

## 6. 当前已测状态

截至 2026-07-13：

- 用户已在小米手机上安装 debug APK。
- 登录/注册已成功。
- Supabase 云同步任务已成功。
- 添加带日期时间的任务已成功。
- 提醒开关可以保存，但还不会真正触发系统通知。
...
- 不适合正式发布给大量用户
- 体积会比正式 release 包大
- 当前还没有接入 Android 系统通知，不能真正到点弹提醒
