...
- 当前最合理的下一步：让用户在手机或电脑浏览器打开 Web 版地址，登录测试公开网页与 Android/Windows 的同步。
...
- Web 版地址：https://zzldbq.github.io/simple-todo/
- 正式版本：`v2.0.0`
...
- v3.0 Web 已通过 GitHub Pages 部署，地址为 `https://zzldbq.github.io/simple-todo/`；部署流程见 `.github/workflows/deploy-web.yml` 和 `docs/WEB_DEPLOYMENT.md`。
...
- 2026-07-15：朋友使用公开 Web 版注册账号后添加任务失败，页面提示 `Auth session missing!`。原因是 Supabase 开启邮箱确认时，注册接口可能返回用户信息但不返回有效登录 session，旧逻辑误把这种状态当作已登录。已修复 Web 版认证逻辑：注册成功但没有 session 时停留在登录页并提示先确认邮箱；读取当前用户和添加任务改为检查 Supabase session；`npm run lint` 与带 `/simple-todo/` base path 的 `npm run build` 已通过。
- 2026-07-14：启用 GitHub Pages，并设置为 GitHub Actions 部署来源；设置仓库变量 `VITE_SUPABASE_URL` 与 `VITE_SUPABASE_ANON_KEY`；重新触发 `Deploy Web` 工作流并部署成功。公开访问地址为 `https://zzldbq.github.io/simple-todo/`，已用 HTTP 访问确认返回 `200 OK`。
...
下一步：让用户在电脑浏览器、安卓手机浏览器或 iPhone Safari 打开 `https://zzldbq.github.io/simple-todo/`，登录同一 Supabase 账号，验证公开 Web 版与 Android/Windows 同步。
