...
- 当前阶段状态：`v2.0.0` 已发布到 GitHub Release。
- 用户不手写代码，希望亲自运行和测试；每个实质步骤更新本文件，并简要解释新工具及 2～4 个同类工具。
- 当前最合理的下一步：启动 v3.0 Web 版设计与开发准备，让 Windows / Android / Web 共用 Supabase。
...
- v3.0 规划为 Web 版：`docs/V3_WEB_PLAN.md`，目标是 Windows / Android / Web 三端互联。
...
2. 创建 Web 项目目录，例如 `web_app`。
3. 接入 Supabase Auth。
4. 实现任务列表、添加、完成、删除、手动刷新。
5. 在 Windows 浏览器和 iPhone Safari 中测试。
6. 部署到 Web 托管平台。
...
下一步：继续完善 v2.1 方向：Android 到点系统通知、正式 release 签名 APK、更多自动化测试、Windows 与 Android 功能一致性对照表。
