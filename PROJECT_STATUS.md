...
- 当前 Android 阶段下一步：接入手机本地通知，实现到点提醒。
- 用户不手写代码，希望亲自运行和测试；每个实质步骤更新本文件，并简要解释新工具及 2～4 个同类工具。
- 当前最合理的下一步：在 Android 端接入系统通知；随后让 Windows 端接入 Supabase，实现电脑与手机真正双向同步。
...
- 2.0 已创建 Android 手机端 Flutter 项目，已完成登录/注册、任务增删改查、Supabase 云同步、日期时间与提醒开关保存。
...
新对话开始时先读取本文件，并检查 Git 当前分支和工作区状态。用户说“继续开发 2.0”后，优先继续 Android 本地通知/到点提醒；随后推进 Windows 端接入 Supabase，实现电脑与手机真正双向同步。Android debug APK 已能构建，当前 APK 路径为 `mobile_app\build\app\outputs\flutter-apk\app-debug.apk`。
...
下一步：Android 端接入系统通知，实现到点提醒；随后给 Android 端补关键自动化测试，并开始让 Windows 端接入 Supabase，实现电脑与手机双向同步。
