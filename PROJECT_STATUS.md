...
- 当前阶段下一步：用户安装新版 Android APK，测试手机刷新按钮与 Windows/Android 双向同步；测试通过后再打标签并发布 `v2.0.0`。
- 用户不手写代码，希望亲自运行和测试；每个实质步骤更新本文件，并简要解释新工具及 2～4 个同类工具。
- 当前最合理的下一步：安装新版 `mobile_app\build\app\outputs\flutter-apk\app-debug.apk`，运行 `dist\SimpleTodo.exe`，用同一 Supabase 账号测试电脑和手机互相刷新任务。
...
- 2.0 已创建 Android 手机端 Flutter 项目，已完成登录/注册、任务增删改查、Supabase 云同步、日期时间与提醒开关保存、手机端手动刷新按钮；Windows 端已加入 Supabase 登录与任务同步入口，待用户实测双向同步。
...
下一步：用户安装新版 Android APK 并测试 Windows 与 Android 双向同步。测试方式：先在手机添加一条任务；打开 `dist\SimpleTodo.exe`，确保 EXE 同目录有 `.env`，用同一账号登录并查看是否出现手机任务；再在 Windows 添加一条任务，回手机端点击右上角刷新按钮或下拉刷新，确认是否出现。通过后再提交、打标签并发布 `v2.0.0`。
