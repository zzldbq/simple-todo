...
- 当前阶段状态：`v3.0.0` Web 版基础工程已创建，登录、注册、任务增删改查、完成/删除、手动刷新等首版代码已完成，并通过 Web 构建验证。
- 用户不手写代码，希望亲自运行和测试；每个实质步骤更新本文件，并简要解释新工具及 2～4 个同类工具。
- 当前最合理的下一步：让用户在电脑浏览器运行 `web_app`，用同一个 Supabase 账号测试 Web 与 Windows/Android 的同步。
...
下一步：运行 `web_app` 本地开发服务器，让用户在电脑浏览器登录测试；随后用手机浏览器访问同一 Web 页面，验证 Web 与 Android/Windows 三端同步。
